...
!                   2004/08/01, 2004/08/20, 2006/05/12, 2006/09/21,
...
!***********************************************************************
      subroutine s_lspp(fpgpvvar,fplspopt,fpwdnews,fplsnews,fplspsmt,   &
     &                  gtinc,ni,nj,nk,jcb,ppp,rbcxy,ppgpv,pptd,pfrc,   &
     &                  tmp1)
!***********************************************************************
...
! Damp to the GPV data.
        ...
        if(gpvvar(8:8).eq.'o'.and.mod(lspopt,10).eq.1) then
...
!$omp do schedule(runtime) private(i,j)
            ...
            do j=1,nj-1
            do i=1,ni-1
              tmp1(i,j,k)=jcb(i,j,k)                                    &
     &          *(ppp(i,j,k)-(ppgpv(i,j,k)+pptd(i,j,k)*gtinc))
            end do
            end do
...
!$omp do schedule(runtime) private(i,j)
            ...
            do j=1,nj-1
            do i=1,ni-1
              tmp1(i,j,k)=jcb(i,j,k)*ppp(i,j,k)
            end do
            end do
...
!$omp do schedule(runtime) private(i,j)
            ...
            do j=2,nj-2
            do i=2,ni-2
              pfrc(i,j,k)=pfrc(i,j,k)-lsnews*rbcxy(i,j)*tmp1(i,j,k)
            end do
            end do
...
!$omp do schedule(runtime) private(i,j,a)
            ...
            do i=2,ni-2
              ...
              pfrc(i,j,k)=pfrc(i,j,k)-rbcxy(i,j)*(lsnews*tmp1(i,j,k)    &
