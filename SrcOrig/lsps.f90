...
!***********************************************************************
      subroutine s_lsps(fpgpvvar,fplspopt,fpwdnews,fplsnews,fplspsmt,   &
     &                  apg,gtinc,ni,nj,nk,rst,sp,rbcxy,sgpv,std,sfrc,  &
     &                  tmp1)
!***********************************************************************
...
! Damp to the GPV data.
        ...
        if(gpvvar(apg:apg).eq.'o'.and.mod(lspopt,10).eq.1) then
...
!$omp do schedule(runtime) private(i,j)
     ...
     &          *(sp(i,j,k)-(sgpv(i,j,k)+std(i,j,k)*gtinc))
            end do
            end do
