...
!     and concentrations but no temperature variation for evaporation.
...
! Exceptional access control
      ...
      public :: newblk_noevap, s_newblk_noevap
...
! Module procedure
      ...
      interface newblk_noevap

        module procedure s_newblk_noevap
...
!***********************************************************************
      subroutine s_newblk_noevap(cphopt,thresq,                         &
...
!$omp&   private(nmci,mssr,msgr,clnmci,cfmsxr,qvsink,qxsink,dqv)
...
! Get the new potential temperature perturbation.
            ...
            ptpf(i,j,1)=ptpf(i,j,1)+(max(vdvr(i,j,1),0.e0)*lv(i,j,1)    &
     &        +(clnmci+cfmsxr)*lf(i,j,1)+(max(vdvi(i,j,1),0.e0)         &
     &        +max(vdvs(i,j,1),0.e0)+max(vdvg(i,j,1),0.e0)              &
     &        +nuvi(i,j,1))*ls(i,j,1))*cppiv
...
! ice mixing ratio.
            ...
            qvsink=nuvi(i,j,1)                                          &
     &        +vdvr(i,j,1)+vdvi(i,j,1)+vdvs(i,j,1)+vdvg(i,j,1)
...
! Get the new potential temperature perturbation.
              ...
              ptpf(i,j,k)=ptpf(i,j,k)+(max(vdvr(i,j,k),0.e0)*lv(i,j,k)  &
     &          +(clnmci+cfmsxr)*lf(i,j,k)+(max(vdvi(i,j,k),0.e0)       &
     &          +max(vdvs(i,j,k),0.e0)+max(vdvg(i,j,k),0.e0)            &
     &          +nuvi(i,j,k))*ls(i,j,k))*cppiv
...
! ice mixing ratio.
              ...
              qvsink=nuvi(i,j,k)                                        &
     &          +vdvr(i,j,k)+vdvi(i,j,k)+vdvs(i,j,k)+vdvg(i,j,k)
...
!!!! -----
      ...
      end subroutine s_newblk_noevap

!-----7--------------------------------------------------------------7--

      end module m_newblk_noevap
