...
!***********************************************************************
      subroutine s_pblptv(fplevpbl,fpdziv,dtb,ni,nj,nk,jcb8w,rbr,rst,   &
     &                    ct,khs,ptv,rr,ss,tt,tmp1)
!***********************************************************************
...
!$omp do schedule(runtime) private(i,j)
        ...
        do j=1,nj-1
        do i=1,ni-1
          ss(i,j,2)=1.e0+ct(i,j)/rst(i,j,2)*dtdzv
        end do
        end do
...
!$omp do schedule(runtime) private(i,j,a)
        ...
        do i=1,ni-1
          ...
          rr(i,j,2)=0.e0
          ss(i,j,2)=1.e0+a*(ct(i,j)*dtdzv-tmp1(i,j,3))
...
!$omp do schedule(runtime) private(i,j)
      ...
      do j=1,nj-1
      do i=1,ni-1
        ptv(i,j,2)=ptv(i,j,2)+ct(i,j)*ptv(i,j,1)/rst(i,j,2)*dtdzv
      end do
      end do
...
! Set the bottom boundary conditions.
      ...
      call vbcs(ni,nj,nk,ptv)

! -----

!! -----

      end subroutine s_pblptv
