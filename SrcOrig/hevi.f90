...
!     Date        : 1999/12/17
!     Modification: 1999/12/20, 2000/01/17, 2000/03/17, 2000/04/18,
!                   2000/06/01, 2000/12/18, 2001/01/15, 2001/03/13,
!                   2001/05/29, 2001/06/06, 2001/06/29, 2001/07/13,
!                   2001/08/07, 2001/09/13, 2001/11/20, 2002/04/02,
!                   2002/07/23, 2002/08/15, 2003/01/04, 2003/03/21,
!                   2003/04/30, 2003/05/19, 2003/11/05, 2003/11/28,
!                   2003/12/12, 2003/12/26, 2004/04/15, 2004/05/31,
!                   2004/06/10, 2004/07/01, 2005/01/31, 2006/04/03,
!                   2006/06/21, 2006/11/06, 2007/01/20, 2007/05/07,
!                   2008/05/02, 2008/08/25, 2008/12/11, 2009/01/30,
!                   2009/02/27, 2009/03/23, 2011/09/22, 2013/02/13

!-----7--1----+----2----+----3----+----4----+----5----+----6----+----7--

! In this module,
!     control the inferior procedures for the horizontally explicit and
!     vertically implicit method.
...
!***********************************************************************
      subroutine s_hevi(fpgwmopt,fpbuyopt,isstp,dts,gtinc,              &
...
! pressure equation.
      ...
      call diverpih(idtrnopt,idmpopt,idmfcopt,iddziv,ni,nj,nk,j31,j32,  &
     &              jcb8u,jcb8v,mf,rmf,rmf8u,rmf8v,rcsq,uf,vf,tmp2,     &
     &              tmp4,tmp5,tmp6)

      call diverpiv(iddziv,ni,nj,nk,rcsq,wf,tmp4)

      call advbspi(idweicoe,'back',ni,nj,nk,rst,wf,pfrc,tmp2,tmp4,tmp1)

! -----

! Calculate the pressure gradient force with the implicit method.

      call pgradiv(iddziv,idweicoe,dts,ni,nj,nk,jcb,wfrc,tmp1,tmp3,tmp4)
...
! Calculate the buoyancy in the small time steps.
      ...
      if(buyopt.eq.1) then
        ...
        call buoywsi(idgwmopt,idweicoe,dts,ni,nj,nk,rbr,ptbr,rst,rcsq,  &
     &               ppf,ptpf,tmp1,tmp3,tmp4)
...
! eqation.
        ...
        call advbspt(idgwmopt,iddziv,ni,nj,nk,ptbr,rbr,wf,tmp2,tmp4)
...
! Solve the potential temperature perturbation to the next time step.
     ...
     &               rst,ptfrc,tmp2,ptcpx,ptcpy,ptpgpv,ptptd,ptpf)
...
! Solve the z components of velocity to the next time step.
      ...
      call stepwi(idexbvar,idexbopt,idimpopt,idbuyopt,iddziv,idweicoe,  &
     &            isstp,dts,gtinc,ni,nj,nk,j31,j32,jcb,jcb8w,mf,        &
     &            rbr,rst,rst8w,rcsq,uf,vf,wcpx,wcpy,wgpv,wtd,          &
     &            tmp3,wf,wc,tmp4,tmp5,tmp6)
...
! pressure equation.
      ...
      call advbspi(idweicoe,'fore',ni,nj,nk,rst,wf,pfrc,tmp3,tmp4,tmp1)

! -----

! Solve the pressure perturbation to the next time step.

      call steppi(idexbvar,idexbopt,isstp,dts,gtinc,ni,nj,nk,jcb,tmp1,  &
     &            pcpx,pcpy,ppgpv,pptd,ppf)

! -----

      end subroutine s_hevi
