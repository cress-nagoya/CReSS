...
!                   2001/05/29, 2001/11/20, 2002/04/02, 2002/06/06,
!                   2002/06/18, 2003/04/30, 2003/05/19, 2003/10/31,
!                   2003/12/12, 2004/05/07, 2004/08/01, 2004/09/10,
!                   2005/02/10, 2006/09/21, 2007/01/20, 2007/05/14,
!                   2007/10/19, 2008/05/02, 2008/06/09, 2008/08/19,
...
!***********************************************************************
      subroutine s_hint2d(fpmpopt,fpintopt,xo,fproc,                    &
     &                    ni,nj,ri,rj,di,dj,var,nid,njd,dat)
!***********************************************************************
...
! Input variables
      ...
      real, intent(in) :: dat(1:nid,1:njd)
...
! Output variable
      ...
      real, intent(out) :: var(0:ni+1,0:nj+1)
     ...
     &        .or.jdmin-1.lt.1.or.jdmax+2.gt.njd) then
        ...
        if(stat.lt.0) then
          ...
          if(mype.eq.-stat-1) then
            ...
            call destroy('hint2d  ',6,'cont',7,'              ',14,101, &
     &                   stat)
          ...
          call destroy('hint2d  ',6,'stop',1001,'              ',14,101,&
     &                 stat)
...
!$omp do schedule(runtime) private(i,j,id,jd,di1,xint1,xint2)
          ...
          do i=istr,iend
            ...
            xint1=di1*dat(id,jd)+di(i,j)*dat(id+1,jd)
            xint2=di1*dat(id,jd+1)+di(i,j)*dat(id+1,jd+1)

            var(i,j)=(1.e0-dj(i,j))*xint1+dj(i,j)*xint2
...
!$omp do schedule(runtime) private(i,j,id,jd,idp1,di1,xint1,xint2)
          ...
          do i=istr,iend
            ...
            xint1=di1*dat(id,jd)+di(i,j)*dat(idp1,jd)
            xint2=di1*dat(id,jd+1)+di(i,j)*dat(idp1,jd+1)
...
!$omp&   private(i,j,id,jd,di1,dj1,di2,di12,xint1,xint2,xint3,xint4,a,b)
          ...
          do i=istr,iend
            ...
            a=(.5e0*(dat(id-1,jd-1)+dat(id+1,jd-1))-dat(id,jd-1))*di2   &
     &        +.5e0*(dat(id+1,jd-1)-dat(id-1,jd-1))*di(i,j)+dat(id,jd-1)

            b=(.5e0*(dat(id,jd-1)+dat(id+2,jd-1))-dat(id+1,jd-1))*di12  &
     &        +.5e0*(dat(id,jd-1)-dat(id+2,jd-1))*di1+dat(id+1,jd-1)

            xint1=di1*a+di(i,j)*b

            a=(.5e0*(dat(id-1,jd)+dat(id+1,jd))-dat(id,jd))*di2         &
     &        +.5e0*(dat(id+1,jd)-dat(id-1,jd))*di(i,j)+dat(id,jd)

            b=(.5e0*(dat(id,jd)+dat(id+2,jd))-dat(id+1,jd))*di12        &
     &        +.5e0*(dat(id,jd)-dat(id+2,jd))*di1+dat(id+1,jd)
     ...
     &        +.5e0*(dat(id,jd+1)-dat(id+2,jd+1))*di1+dat(id+1,jd+1)
            ...
            a=(.5e0*(dat(id-1,jd+2)+dat(id+1,jd+2))-dat(id,jd+2))*di2   &
     &        +.5e0*(dat(id+1,jd+2)-dat(id-1,jd+2))*di(i,j)+dat(id,jd+2)

            b=(.5e0*(dat(id,jd+2)+dat(id+2,jd+2))-dat(id+1,jd+2))*di12  &
     &        +.5e0*(dat(id,jd+2)-dat(id+2,jd+2))*di1+dat(id+1,jd+2)
...
!$omp&   private(di1,dj1,di2,di12,xint1,xint2,xint3,xint4,a,b)
          ...
          do i=istr,iend
            ...
            a=(.5e0*(dat(idm1,jd-1)+dat(idp1,jd-1))-dat(id,jd-1))*di2   &
     &        +.5e0*(dat(idp1,jd-1)-dat(idm1,jd-1))*di(i,j)+dat(id,jd-1)

            b=(.5e0*(dat(id,jd-1)+dat(idp2,jd-1))-dat(idp1,jd-1))*di12  &
     &        +.5e0*(dat(id,jd-1)-dat(idp2,jd-1))*di1+dat(idp1,jd-1)

            xint1=di1*a+di(i,j)*b

            a=(.5e0*(dat(idm1,jd)+dat(idp1,jd))-dat(id,jd))*di2         &
     &        +.5e0*(dat(idp1,jd)-dat(idm1,jd))*di(i,j)+dat(id,jd)

            b=(.5e0*(dat(id,jd)+dat(idp2,jd))-dat(idp1,jd))*di12        &
     &        +.5e0*(dat(id,jd)-dat(idp2,jd))*di1+dat(idp1,jd)
     ...
     &        +.5e0*(dat(id,jd+1)-dat(idp2,jd+1))*di1+dat(idp1,jd+1)
            ...
            a=(.5e0*(dat(idm1,jd+2)+dat(idp1,jd+2))-dat(id,jd+2))*di2   &
     &        +.5e0*(dat(idp1,jd+2)-dat(idm1,jd+2))*di(i,j)+dat(id,jd+2)

            b=(.5e0*(dat(id,jd+2)+dat(idp2,jd+2))-dat(idp1,jd+2))*di12  &
     &        +.5e0*(dat(id,jd+2)-dat(idp2,jd+2))*di1+dat(idp1,jd+2)
