...
!                   2008/08/25, 2009/02/27, 2011/03/18, 2011/08/18,
...
!***********************************************************************
      subroutine s_termbw2d(ni,nj,nqw,rbw,rrbw,rbr,rbv,t,ubw,ubwmax,    &
     &                      c1,c2,c3,c4,c5)
!***********************************************************************
...
! Input variables
      ...
      real, intent(in) :: rbr(0:ni+1,0:nj+1)
...
! Output variable
      ...
      real, intent(out) :: ubw(0:ni+1,0:nj+1,1:nqw)
...
! Internal shared variables
      ...
      real, intent(inout) :: ubwmax(0:ni+1,0:nj+1)
      ...
      real, intent(inout) :: c4(0:ni+1,0:nj+1)
                       ! Common used temporary array

      real, intent(inout) :: c5(0:ni+1,0:nj+1)
                       ! Common used temporary array
...
!$omp do schedule(runtime) private(i,j,a,b,c,d)
      ...
      do i=1,ni-1
        ...
        a=rhow03-rbr(i,j)
        b=76.1e0-.155e0*(t(i,j)-t0)

        c=exp(oned6*log(b*b*b*geta4v*rbr(i,j)*rbr(i,j)/a))

        c1(i,j)=geta29*a
        c2(i,j)=etav48*c1(i,j)*rbr(i,j)
        c3(i,j)=eta05*rbv(i,j)
        c4(i,j)=g163*a*c/b
        c5(i,j)=c*c3(i,j)
     ...
     &    +.238449e-2*b*c-.542819e-1*b*b
        ...
        ubwmax(i,j)=rbwmiv*exp(d)*c5(i,j)
...
!$omp do schedule(runtime) private(i,j,a,b,c,d)
        ...
        do i=1,ni-1
          ...
          if(rbw(n).lt.1.e-3) then

            ubw(i,j,n)=c1(i,j)*rrbw(n,4)

          else if(rbw(n).ge.1.e-3.and.rbw(n).lt.5.35e-2) then

            a=log(c2(i,j)*rrbw(n,2))
     ...
     &        +.855176e-4*b*c-.578878e-3*b*b-.327815e-5*c*c
            ...
            ubw(i,j,n)=exp(d)*c3(i,j)*rrbw(n,5)

          else if(rbw(n).ge.5.35e-2.and.rbw(n).lt..35e0) then

            a=log(c4(i,j)*rrbw(n,1))
     ...
     &        +.238449e-2*b*c-.542819e-1*b*b
            ...
            ubw(i,j,n)=exp(d)*c5(i,j)*rrbw(n,3)

          else

            ubw(i,j,n)=ubwmax(i,j)
...
!! -----
      ...
      end subroutine s_termbw2d

!-----7--------------------------------------------------------------7--

      end module m_termbw2d
