...
!     Modification: 2024/12/25  new dmpvar entry
...
! Dump the base state potential temperature and its perturbation.
     ...
     &                      ni,nj,nk,ptp)
            ...
            else if(dmpvar(5:5).eq.'+'.or.dmpvar(5:5).eq.'-') then
...
! Dump the heating rate for cloud physics.

            if(abs(cphopt).le.4) then

              if((dmpvar(15:15).eq.'o'.or.dmpvar(15:15).eq.'+'.or.      &
     &            dmpvar(15:15).eq.'-')) then

                call s_dmpcld( ni, nj, nk, dqdt, 'dqdt ', 'o', dtb )
                call s_dmpcld( ni, nj, nk, dqadt, 'dqadt', 'o', dmpitv )

                call outdmp3d('dqdt  ',4,capt(151),ncpt(151),'xxx',     &
     &                        ni,nj,nk,dqdt)

                call outdmp3d('dqadt ',5,capt(152),ncpt(152),'xxx',     &
     &                        ni,nj,nk,dqadt)

                call setcst3d(0,ni+1,0,nj+1,1,nk,0.0e0,dqdt)
                call setcst3d(0,ni+1,0,nj+1,1,nk,0.0e0,dqadt)
...
! Dump forcing for turbulence.


            if(dmpvar(16:16).eq.'o'.or.dmpvar(16:16).eq.'+') then

              call outdmp3d('turbpt',6,capt(155),ncpt(155),'xxx',       &
     &                      ni,nj,nk,turbpt)
            ...
            if(dmpvar(16:16).eq.'+') then
              ...
              call outdmp3d('numdpt',6,capt(167),ncpt(167),'xxx',       &
     &                      ni,nj,nk,numdpt)
...
! Dump the Asselin Filter.


            if(dmpvar(5:5).eq.'+')then

              call outdmp3d('fltpt ',5,capt(161),ncpt(161),             &
     &                      'xxx',ni,nj,nk,dmpfltpt)
...
! Dump the heating rate for cloud physics.
     ...
     &            dmpvar(15:15).eq.'-')) then
                ...
                call vint31s(ni,nj,nk,z1d,zph8s,dqdt,var)

                call outdmp3d('dqdt  ',4,capt(151),ncpt(151),'xxx',     &
     &                        ni,nj,nk,var)

                call vint31s(ni,nj,nk,z1d,zph8s,dqadt,var)
...
! Dump forcing for turbulence.
            ...
            if(dmpvar(16:16).eq.'o'.or.dmpvar(16:16).eq.'+') then
              ...
              call vint31s(ni,nj,nk,z1d,zph8s,turbpt,var)
              call outdmp3d('turbpt',6,capt(155),ncpt(155),'xxx',       &
     &                      ni,nj,nk,var)
            ...
            if(dmpvar(16:16).eq.'+') then
              ...
              call vint31s(ni,nj,nk,z1d,zph8s,numdpt,var)
              call outdmp3d('numdpt',6,capt(167),ncpt(167),'xxx',       &
     &                      ni,nj,nk,var)
...
! Dump the Asselin Filter.
            ...
            if(dmpvar(5:5).eq.'+')then
              ...
              call vint31s(ni,nj,nk,z1d,zph8s,dmpfltpt,var)
...
! Dump the z physical coordinates on terrain following coordinates.
        ...
        if(fdmp(1:3).eq.'act') then
          ...
          if(mod(dmplev,10).eq.1.and.dmpvar(17:17).eq.'o') then
