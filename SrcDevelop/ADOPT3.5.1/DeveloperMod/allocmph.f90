...
!-- Monitoring variables for cloud microphysics
      allocate(dqdt(0:ni+1,0:nj+1,1:nk),stat=cstat)

      stat=stat+abs(cstat)

      allocate(vdvc(0:ni+1,0:nj+1,1:nk),stat=cstat)

      stat=stat+abs(cstat)

      allocate(dqadt(0:ni+1,0:nj+1,1:nk),stat=cstat)

      stat=stat+abs(cstat)

      allocate(vdvc2(0:ni+1,0:nj+1,1:nk),stat=cstat)

      stat=stat+abs(cstat)

      call setcst3d(0,ni+1,0,nj+1,1,nk,0.e0,dqdt)
      call setcst3d(0,ni+1,0,nj+1,1,nk,0.e0,dqadt)
      call setcst3d(0,ni+1,0,nj+1,1,nk,0.e0,vdvc)
      call setcst3d(0,ni+1,0,nj+1,1,nk,0.e0,vdvc2)
