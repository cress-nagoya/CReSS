...
! In this module,
!     generate the united geography file from the dumped files in entire
!     domain.
...
!***********************************************************************
      subroutine s_unigeoen(fpexprim,fpcrsdir,fpncexp,fpnccrs,fpwlngth, &
...
! Open the united file.
  ...
  100 if(stat.ne.0) then
        ...
        call destroy('unigeoen',8,'stop',1,'              ',14,iouni,   &
     &               stat)

      end if

      call outstd03('unigeoen',8,unifl,ncexp+20,iouni,1,0,0_i8)
...
! Open the dumped file.
  ...
  110       if(stat.ne.0) then
              ...
              call destroy('unigeoen',8,'cont',1,'              ',14,   &
     &                     iodmp(0),stat)
            ...
            call outstd03('unigeoen',8,dmpfl,ncexp+29,iodmp(0),1,0,0_i8)

          else

            write(dmpfl(ncexp+1:ncexp+33),'(a13,i9.9,a11)')             &
     &                               'geography.grp',mygrp,'.united.bin'

            if(rmopt_uni.eq.1) then

              open(iodmp(0),iostat=stat,err=120,                        &
     &             file=crsdir(1:nccrs)//dmpfl(1:ncexp+33),             &
     &             status='old',access='direct',form='unformatted',     &
     &             recl=sizdmp,action='readwrite')
  ...
  120       if(stat.ne.0) then
     ...
     &                     iodmp(0),stat)
            ...
            call outstd03('unigeoen',8,dmpfl,ncexp+28,iodmp(0),1,0,0_i8)
...
! united file.
  ...
  130     if(recdmp.eq.1) then
            ...
            call destroy('unigeoen',8,'cont',3,'              ',14,     &
     &                   iodmp(0),stat)
            ...
            call outstd03('unigeoen',8,dmpfl,108,iodmp(0),3,0,0_i8)

          end if

          call outstd03('unigeoen',8,unifl,108,iouni,4,0,0_i8)
...
! Close the dumped file.
  ...
  140     if(stat.ne.0) then
            ...
            call destroy('unigeoen',8,'cont',2,'              ',14,     &
     &                   iodmp(0),stat)
          ...
          call outstd03('unigeoen',8,unifl,108,iodmp(0),2,0,0_i8)
...
! Close the united file.
  ...
  150 if(stat.ne.0) then
        ...
        call destroy('unigeoen',8,'stop',2,'              ',14,iouni,   &
     &               stat)

      end if

      call outstd03('unigeoen',8,unifl,108,iouni,2,0,0_i8)
...
! If error occured, call the procedure destroy.
  ...
  160 call destroy('unigeoen',8,'cont',4,'              ',14,iouni,     &
     &             stat)
  ...
  180 if(stat.ne.0) then
     ...
     &               stat)
      ...
      call destroy('unigeoen',8,'stop',1001,'              ',14,101,    &
     &             stat)

! -----

      end subroutine s_unigeoen

!-----7--------------------------------------------------------------7--

      end module m_unigeoen
