...
! In this module,
!     generate the united history file from the dumped files in entire
!     domain.
...
!***********************************************************************
      subroutine s_unidmpen(fpexprim,fpcrsdir,fpncexp,fpnccrs,fpwlngth, &
     &                      fprmopt_uni,ctime,nx,ny,var,nio_uni,iodmp)
!***********************************************************************
...
! Open the united file.
     ...
     &     file=crsdir(1:nccrs)//unifl(1:ncexp+22),                     &
     &     status='new',access='direct',form='unformatted',             &
     &     recl=sizuni,action='write')

  100 if(stat.ne.0) then

        call destroy('unidmpen',8,'stop',1,'              ',14,iouni,   &
     &               stat)

      end if

      call outstd03('unidmpen',8,unifl,ncexp+22,iouni,1,1,ctime)
...
! Open the dumped file.
     ...
     &                'dmp',ctime/1000_i8,'.grp',mygrp,'-sub0000.bin'
  ...
  110       if(stat.ne.0) then
              ...
              call destroy('unidmpen',8,'cont',1,'              ',14,   &
     &                     iodmp(0),stat)
            ...
            call outstd03('unidmpen',8,dmpfl,ncexp+31,iodmp(0),1,1,     &
     &                    ctime)
  ...
  120       if(stat.ne.0) then
     ...
     &                     iodmp(0),stat)
            ...
            call outstd03('unidmpen',8,dmpfl,ncexp+30,iodmp(0),1,1,     &
     &                    ctime)
...
! united file.
  ...
  130     if(recdmp.eq.1) then
            ...
            call destroy('unidmpen',8,'cont',3,'              ',14,     &
     &                   iodmp(0),stat)
            ...
            call outstd03('unidmpen',8,dmpfl,108,iodmp(0),3,1,ctime)

          end if

          call outstd03('unidmpen',8,unifl,108,iouni,4,1,ctime)
...
! Close the dumped file.
  ...
  140     if(stat.ne.0) then
            ...
            call destroy('unidmpen',8,'cont',2,'              ',14,     &
     &                   iodmp(0),stat)
          ...
          call outstd03('unidmpen',8,unifl,108,iodmp(0),2,1,ctime)
...
! Close the united file.
  ...
  150 if(stat.ne.0) then
        ...
        call destroy('unidmpen',8,'stop',2,'              ',14,iouni,   &
     &               stat)

      end if

      call outstd03('unidmpen',8,unifl,108,iouni,2,1,ctime)
...
! If error occured, call the procedure destroy.
  ...
  160 call destroy('unidmpen',8,'cont',4,'              ',14,iouni,     &
     &             stat)
  ...
  180 if(stat.ne.0) then
     ...
     &               stat)
      ...
      call destroy('unidmpen',8,'stop',1001,'              ',14,101,    &
     &             stat)

! -----

      end subroutine s_unidmpen

!-----7--------------------------------------------------------------7--

      end module m_unidmpen
